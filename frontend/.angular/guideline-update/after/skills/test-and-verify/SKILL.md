...
## Evidence for interactive changes

- Exercise the rendered trigger and resulting UI, not only a mocked handler call. For forms or dialogs, cover the affected activation paths, state change, cancellation, and repeat/open-close behavior in proportion to risk.
- If a regression involves remounting, focus loss, or reset state, check stable control identity across a bounded number of update cycles as well as the displayed values.
- If asynchronous scheduling or change detection is involved, verify the real update lifecycle with automatic updates and a bounded stability/settling check. Manual refresh after every action can hide the defect; an unbounded pre-fix loop can hang the test runner.
- Treat typecheck/build, component tests, and browser checks as different coverage. A success toast, HTTP 200, or automation timeout does not establish full workflow success. Confirm actions took effect and report uncertain observations.
- When safe and practical, use the same focused regression case before and after the fix. Record browser/runtime and executed/skipped cases. If the normal runner is unavailable, use a suitable already-installed alternative when possible and disclose its coverage limits; otherwise mark runtime checks `BLOCKED` or `NOT TESTED`.
