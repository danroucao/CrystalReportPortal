...
## Runtime-stability guidance update

2026-09-12: Added portable interaction and evidence safeguards to `feature-development`, `frontend-design`, `debug`, `test-and-verify`, `code-review`, and `global/AGENTS.md`. Skill responsibilities and invocation metadata are unchanged. The conditional [reactive UI diagnosis reference](skills/debug/references/reactive-ui-failures.md) covers identity/remount feedback, evidence limits, and bounded regression checks; Angular-specific APIs remain examples rather than global requirements.
